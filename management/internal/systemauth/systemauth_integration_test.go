...
func TestPAMSystemAccountLifecycleIntegration(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("correct system password rejected before normal change: %v", err)
	}
	...
	if err := ChangePassword(username, oldPassword, newPassword); err != nil {
		t.Fatalf("normal PAM password change failed: %v", err)
	}
	if _, err := Authenticate(username, oldPassword); !errors.Is(err, ErrInvalidCredentials) {
		t.Fatalf("old password still authenticates after normal change: %v", err)
	}
	result, err = Authenticate(username, newPassword)
	if err != nil {
		t.Fatalf("new system password rejected after normal PAM change: %v", err)
	}
	if result.PasswordChangeRequired {
		t.Fatal("new password unexpectedly requires a change")
	}
	...
	if err := ChangePassword(username, newPassword, finalPassword); err != nil {
		t.Fatalf("expired PAM password change failed: %v", err)
	}
	if _, err := Authenticate(username, newPassword); !errors.Is(err, ErrInvalidCredentials) {
		t.Fatalf("pre-expiration password still authenticates after change: %v", err)
	}
	result, err = Authenticate(username, finalPassword)
	if err != nil {
		t.Fatalf("final system password rejected after expired PAM change: %v", err)
	}
	if result.PasswordChangeRequired {
		t.Fatal("final password still marked as expired")
	}
}
