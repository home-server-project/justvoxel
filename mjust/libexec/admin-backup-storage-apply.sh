...
apply_json() {
    ...
    if [[ ${TARGET_TYPE} == partition && ${TARGET_ALREADY_MOUNTED} != true ]]; then
        ...
        mount "${TARGET_MOUNT}" >/dev/null 2>&1 || { rollback; json_error 'SMB share could not be mounted. Check the server, share, credentials, network, and permissions.'; return 0; }
        mounted_by_us=yes
        TARGET_EXPECTED_SOURCE="$(findmnt -n -o SOURCE --target "${TARGET_MOUNT}" 2>/dev/null || true)"
